...
public class main {
        ...
        for(int i = 1; i <= 1000; i++) {
            ...
            boolean way = drawBoolean();
            value += findNumber(manager.get(1), t[k], way); 
        }
        value /= 1000;
        ...
        for(int i = 1; i <= 1000; i++) {
            ...
            boolean way = drawBoolean();
            value += findNumber(manager.get(1), k, way); 
        }
        value /= 1000;
    ...
    private static boolean drawBoolean() {
        Random random = new Random();
        return random.nextBoolean();
    }

    private static int findNumber(cyclicList list, int number, boolean way) {
        int counter = 1;
